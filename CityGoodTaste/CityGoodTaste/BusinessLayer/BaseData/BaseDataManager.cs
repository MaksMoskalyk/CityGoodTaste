...
namespace CityGoodTaste.BusinessLayer
{
    public class BaseDataManager : IBaseDataManager
    {
        ...
        public RestaurantSchema GetRestaurantSchema(int? restId)
        {
            using (GoodTasteContext context = new GoodTasteContext())
            {
                try
                {
                    ...
                    Schema = context.RestaurantSchemas.Include(t => t.Restaurant).FirstOrDefault(t => t.Id == restId);
        ...
        public void ReservTables(List<TableReservation> reservs)
        {
            using (GoodTasteContext context = new GoodTasteContext())
            {
                foreach (var item in reservs)
                {
                    ...
                    TableReservation r = new TableReservation { Date = item.Date, Reserved = true, ReservedAndConfirmed = false, User = u, Table = t };
